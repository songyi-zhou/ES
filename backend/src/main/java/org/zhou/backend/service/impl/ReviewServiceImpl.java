...
public class ReviewServiceImpl implements ReviewService {
    ...
    @Override
    @Transactional
    public void batchReject(String formType, String major, String classId, String studentId) {
        log.info("开始批量退回操作 - 评测表类型: {}, 专业: {}, 班级: {}, 学生ID: {}", formType, major, classId, studentId);
            ...
            for (String materialIds : materialIdsList) {
                if (materialIds != null && !materialIds.isEmpty()) {
                    ...
                    String updateMaterialsSql = """
                        ...
                        SET status = 'UNCORRECT', score = 0
                        WHERE id IN (%s) 
                        AND status NOT IN ('DEDUCTED', 'PUNISHED')
                    """.formatted(inClause);
                    log.info("执行材料更新SQL: {}", updateMaterialsSql);
                    int updatedMaterials = jdbcTemplate.update(updateMaterialsSql);
